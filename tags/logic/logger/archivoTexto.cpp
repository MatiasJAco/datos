...
void ArchivoTexto::leerCaracter(char &caracter){
    archivo.get(caracter);
}

bool ArchivoTexto::finDeArchivo(){
     if(archivo.eof())
        return(true);
    else
        return(false);
...
int ArchivoTexto::getPosicionCursor(){
	return archivo.tellp();
};

void ArchivoTexto::setPosicionCursor(int nuevaPosicion){
	archivo.seekp(nuevaPosicion);
};

long ArchivoTexto::getTamanio(){
	int tamanio=0;
